...
![What one head learns, and the order test](image.png)

> **There's a playground for this.** Open [`index.html`](index.html) in any browser (just
> double-click it — no install, no server). Press **Shuffle the tokens** and watch the number stay at
...
Parts [1](../neuron/), [2](../finding-the-weights/) and [3](../depth/)
...
weights and random shufflings ([`src/permutation.py`](src/permutation.py)):
