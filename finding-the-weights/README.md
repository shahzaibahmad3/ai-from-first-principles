...
![60 training runs, identical architecture, different starting weights](image.png)

> **There's a playground for this.** Open [`index.html`](index.html) in any browser (just
> double-click it — no install, no server). Press **Run** and watch a two-unit hidden layer bend a flat
> surface into XOR. Then press **New random init** a few times and watch it fail. Everything below
...
[Part 1](../neuron/) ended at a wall. One neuron computes a weighted sum and fires if it crosses
...
threshold. Run [`src/capacity.py`](src/capacity.py) and it prints the truth table and asserts the result:
...
[`src/credit_assignment.py`](src/credit_assignment.py):
...
about eight minutes in [`src/credit_assignment.py`](src/credit_assignment.py):
