...
The `learning_rate` just sets how big each shove is. And there's a guarantee worth knowing: **if a line separating the two groups exists at all, this stumbling little rule is proven to find one, in a finite number of steps** — the perceptron convergence theorem, proved by Novikoff in 1962, building on the learning rule Rosenblatt introduced in 1958. You don't have to take it on faith — press **Run** on any separable dataset and watch the misses drop to zero.
...
Load **XOR** and press Run. The misses never reach zero. The line lurches back and forth, never settling.
...
The playground and this whole post run on [`src/neuron.py`](src/neuron.py), which uses no libraries at all. Two functions, 23 lines of logic between them. The forward pass:
